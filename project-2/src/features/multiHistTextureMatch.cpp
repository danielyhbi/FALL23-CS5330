...
private:
    ...
    int getSobelMagnitude(Mat &sobelX, Mat &sobelY, Mat &mag)
    {
        ...
        for (int row = 0; row < sobelX.rows; row++)
        {
            ...
            for (int col = 0; col < sobelX.cols; col++)
            {
                ...
                int intensityY1 = rowPointerY[col][1];

                float magnitude = sqrt(intensityX * intensityX + intensityY * intensityY);

                rowPointerM[col] = magnitude;
            }
        }
        ...
        for (int row = startRow; row < endRowNoInclu; row++)
        {
            ...
            for (int col = startCol; col < endColNoInclu; col++)
            {
                for (int channel = 0; channel < 1; channel++)
                {
                    ...
                    rowPointer[col][1] = sum;
                    rowPointer[col][2] = sum;
                }
            }
        }
        ...
        for (int row = 0; row < sobelX.rows; row++)
        {
            ...
            for (int col = 0; col < sobelX.cols; col++)
            {
                for (int channel = 0; channel < 1; channel++)
                {
                    ...
                    rowPointerDst[col][1] = formatRGBValue(sqrt(x * x + y * y));
                    rowPointerDst[col][2] = formatRGBValue(sqrt(x * x + y * y));
                }
            }
        }
